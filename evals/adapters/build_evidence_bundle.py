#!/usr/bin/env python3
"""Build promotion-eligible skill-eval-evidence/v1 from one normalized run.

Executor/LLM judge output is analysis evidence only. This builder fails closed
unless the verifier receipt is deterministic, passed, and bound to the same run.
"""
...
def validate_receipt(run: dict, receipt: dict) -> None:
    ...
    if receipt.get("run_id") != run.get("run_id"):
        raise ValueError("verifier receipt run_id does not match run trace")
    if receipt.get("case_id") != run.get("case_id"):
        raise ValueError("verifier receipt case_id does not match run trace")
    if receipt.get("authority") != "deterministic":
        raise ValueError("promotion evidence requires deterministic verifier authority")
    if receipt.get("passed") is not True:
        raise ValueError("promotion evidence requires a passing verifier receipt")
    verifier = receipt.get("verifier")
    if not isinstance(verifier, dict):
        raise ValueError("verifier receipt missing verifier identity")
    digest = verifier.get("implementation_sha256")
    if not isinstance(digest, str) or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
        raise ValueError("verifier implementation digest must be lowercase sha256")
    input_digest = receipt.get("input_digest")
    if not isinstance(input_digest, str) or len(input_digest) != 64 or any(c not in "0123456789abcdef" for c in input_digest):
        raise ValueError("verifier input_digest must be lowercase sha256")
...
def main() -> int:
    ...
    try:
        ...
        receipt_path = Path(args.verifier_receipt)
        ...
        bundle = {
            ...
            "verifier_receipt_sha256": sha256(receipt_path),

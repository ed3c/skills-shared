...
TITLE = (
    "基於 Antigravity 2.0 與 NotebookLM 代理式架構的數據科學自動化工作流深度研究報告"
)
...
LIBRARIES = [
    {
        "raw_name": "teng-lin/notebooklm-py",
        "name": "notebooklm-py",
...
EXTRA_CONCEPTS = [
    ...
    "Three-Tier Data Workflow",
    "Exploratory Data Analysis",
    "Feature Engineering",
    "Data Pipeline Automation",
    "RAG",
    "Hallucination-Free Knowledge Base",
    "Context Engineering",
...
def main():
    with open(MD, encoding="utf-8") as f:
        text = f.read()
    print(
        f"[S9] extract_concepts via Ollama qwen3:4b（{len(text)} chars 讀入本程序，不進主 context）…"
    )
    ollama_names = extract_concepts(text)  # 同影片軸引擎
    names = list(ollama_names) + EXTRA_CONCEPTS  # UNION S1/verify grounded 概念
    print(
        f"     Ollama {len(ollama_names)} + extra {len(EXTRA_CONCEPTS)} = {len(names)} raw（去重前）"
    )
    ...
    for lib in LIBRARIES:
        ...
        lib_rows.append(
            (lib["raw_name"], lid, "JOIN 既有" if lid in existing else "NEW")
        )
    ...
    print("=" * 68)
    print(
        f"Conversation 節點 : {conv_id}  ({'已存在(idempotent)' if conv_id in existing else 'NEW'})"
    )
    print(
        f"概念總數          : {len(concepts)}  (NEW {len(new_concepts)} / JOIN 既有 {len(join_concepts)})"
    )
    ...
    print(
        f"[ok] {res.conv_node_id}: {len(res.concepts)} concepts, "

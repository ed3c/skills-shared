...
if not commit:
    ...
    print(
        f"[dry-run] conv node: {conv_node} ({'JOIN' if conv_node in existing else 'NEW'})"
    )
    new = joins = dropped = 0
    for c in EXTRA_CONCEPTS:
        ...
        if not cid or cid == "concept:":
            ...
            new += 1
    print(
        f"[dry-run] extra_concepts: {len(EXTRA_CONCEPTS)} → NEW {new} / JOIN {joins} / DROP {dropped}"
    )
    for lib in LIBRARIES:
        lid = library_node_id(lib["raw_name"])
        print(
            f"[dry-run] lib {lib['raw_name']} → {lid} ({'JOIN' if lid in existing else 'NEW'})"
        )

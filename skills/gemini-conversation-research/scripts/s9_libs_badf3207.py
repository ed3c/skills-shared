...
def main():
    ...
    print("=" * 66)
    print(
        f"S9 LIBS — MENTIONS from {conv_id}  ({'存在' if conv_id in existing else 'NEW'})"
    )
    print("=" * 66)
    rows = []
    ...
    print(
        f"估計 node delta ≈ +{new_n}   edge delta ≈ +{len(rows)}（既有 MENTIONS 會 dedup）"
    )
    print("=" * 66)
    ...
    print(
        f"[ok] {res.conv_node_id}: {len(res.libraries)} libraries → "

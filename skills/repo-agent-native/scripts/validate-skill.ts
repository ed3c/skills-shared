...
const HOST_ONLY_FIELDS = new Set([
  "argument-hint", "context", "agent", "model", "disable-model-invocation", "user-invocable",
]);
const REQUIRED_HEADINGS = [
  "Trigger", "Non-trigger", "Inputs", "Outputs", "Core laws", "State machine",
  "S0 — Scope", "S1 — Route", "S2 — Discover", "S3 — Retrieve", "S4 — Verify",
  "S5 — Infer", "S6 — Write", "S7 — Assert", "S8 — Handoff", "Module law",
];
const REQUIRED_MODULE_SECTIONS = [
  "Trigger", "Non-trigger", "Purpose", "Assumptions", "State machine", "Inputs",
  "Outputs and effects", "Evidence class and freshness", "Fallback",
  "Core laws that remain authoritative", "Consumer-owned values",
];
const REQUIRED_TOOL_MODULES = [
  "grepai.md",
  "serena.md",
  "scip.md",
  "tree-sitter.md",
  "sqlite-code-index.md",
  "compiler-truth-context-funnel.md",
  "mem0.md",
];
const REQUIRED_ROUTING_CASES = new Set([
  ...
  "MODULE-SCIP-TRIGGER",
  "MODULE-TREE-SITTER-TRIGGER",
  "MODULE-SQLITE-TRIGGER",
  "MODULE-CONTEXT-FUNNEL-TRIGGER",
  "MODULE-MEM0-TRIGGER",
  "MODULE-CORE-ONLY",
  "MODULE-GREPAI-STALE",
  "MODULE-SERENA-WRONG-PROJECT",
  "MODULE-SCIP-STALE",
  "MODULE-SCIP-COVERAGE-GAP",
  "MODULE-TREE-SITTER-PARSE-ERROR",
  "MODULE-SQLITE-SUBJECT-MISMATCH",
  "MODULE-FUNNEL-READBACK-MISSING",
  "MODULE-MEMORY-CONFLICT",
  "MODULE-AMBIGUOUS-PRIMARY",
  "MODULE-LAW-OVERRIDE",
]);
const ROUTABLE_MODULES = new Set([
  "grepai", "serena", "scip", "tree-sitter", "sqlite-code-index",
  "compiler-truth-context-funnel", "mem0",
]);
...
function checkLinks(skillRoot: string, documentPath: string, text: string, failures: Failure[]): void {
  ...
  for (const match of text.matchAll(linkPattern)) {
    ...
    if (!inside(skillRoot, target)) fail(failures, "BROKEN_RELATIVE_REFERENCE", `link escapes skill root: ${raw}`);
    else if (!existsSync(target)) fail(failures, "BROKEN_RELATIVE_REFERENCE", `link target absent: ${raw}`);
  }
}
...
function checkToolModules(skillRoot: string, failures: Failure[]): void {
  for (const name of REQUIRED_TOOL_MODULES) {
    ...
    if (!existsSync(path)) { fail(failures, "MODULE_CONTRACT_INCOMPLETE", `missing modules/${name}`); continue; }
    const text = readFileSync(path, "utf8");
    for (const heading of REQUIRED_MODULE_SECTIONS) {
      if (!text.includes(`## ${heading}`)) fail(failures, "MODULE_CONTRACT_INCOMPLETE", `modules/${name} missing ${heading}`);
    }
    if (!text.includes("Failure states:")) fail(failures, "MODULE_CONTRACT_INCOMPLETE", `modules/${name} missing Failure states`);
...
function checkRoutingCases(skillRoot: string, failures: Failure[]): void {
  ...
  if (!existsSync(path)) { fail(failures, "MODULE_ROUTING_CASE_INVALID", "missing evals/fixtures/module-routing-cases.json"); return; }
  let value: unknown;
  try { value = JSON.parse(readFileSync(path, "utf8")); }
  catch (error) { fail(failures, "MODULE_ROUTING_CASE_INVALID", `invalid JSON: ${error instanceof Error ? error.message : path}`); return; }
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    fail(failures, "MODULE_ROUTING_CASE_INVALID", "routing fixture must be an object"); return;
  }
  const fixture = value as Record<string, unknown>;
  if (fixture.schema !== "repo-agent-native/module-routing-cases/v1" || !Array.isArray(fixture.cases)) {
    fail(failures, "MODULE_ROUTING_CASE_INVALID", "routing fixture schema/cases are invalid"); return;
  }
  const ids = new Set<string>();
  const positivelyRouted = new Set<string>();
  for (const [index, raw] of fixture.cases.entries()) {
    if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
      fail(failures, "MODULE_ROUTING_CASE_INVALID", `case[${index}] must be an object`); continue;
    }
    const item = raw as Record<string, unknown>;
    if (typeof item.id !== "string" || !item.id || ids.has(item.id)) {
      fail(failures, "MODULE_ROUTING_CASE_INVALID", `case[${index}] has absent or duplicate id`); continue;
    }
    ids.add(item.id);
    if (typeof item.prompt !== "string" || !item.prompt.trim()) fail(failures, "MODULE_ROUTING_CASE_INVALID", `${item.id} has no prompt`);
    if (item.expected_modules !== undefined) {
      if (!Array.isArray(item.expected_modules) || item.expected_modules.some((name) => typeof name !== "string" || !ROUTABLE_MODULES.has(name))) {
        ...
        for (const name of item.expected_modules) positivelyRouted.add(name as string);
      }
    }
  ...
  for (const id of REQUIRED_ROUTING_CASES) if (!ids.has(id)) fail(failures, "MODULE_ROUTING_CASE_INVALID", `required routing case absent: ${id}`);
  for (const name of ROUTABLE_MODULES) if (!positivelyRouted.has(name)) fail(failures, "MODULE_ROUTING_CASE_INVALID", `no positive routing case for ${name}`);
}
...
export function validateSkill(skillRootInput: string): { failures: Failure[]; skillSha256: string } {
  ...
  for (const heading of REQUIRED_HEADINGS) if (!text.includes(`## ${heading}`)) fail(failures, "MISSING_CORE_SECTION", heading);
  ...
  if (/\b(?:grepai|serena|scip|tree-sitter|sqlite-code-index|compiler-truth-context-funnel|code-graph-rag|mem0)\b/i.test(text)) {
    fail(failures, "DOMAIN_INSTANCE_IN_CORE", "provider/capability instances belong in trigger-selected modules, not the portable core");
  }

  for (const path of [
    "scripts/validate-skill.ts", "scripts/assert-output.ts", "scripts/evaluate-retry-predicates.ts",
    "scripts/score-ab-output.ts", "scripts/compare-ab.ts", "scripts/run-ab.ts",
    "tests/selftest.ts", "tests/ab-selftest.ts",
  ]) {
    if (!existsSync(resolve(realRoot, path))) fail(failures, "EXECUTABLE_ASSERTION_ABSENT", path);
  }
  for (const documentPath of activeMarkdownFiles(realRoot)) checkLinks(realRoot, documentPath, readFileSync(documentPath, "utf8"), failures);
...
function parseArgs(args: string[]): CliOptions {
  let skillRoot: string | undefined; let receipt: string | undefined;
...
export function runValidateCli(args: string[]): number {
  try {
    const options = parseArgs(args); const result = validateSkill(options.skillRoot);
    writeReceipt(options.receipt, {
      ...
      state: result.failures.length ? "FAIL" : "PASS", failures: result.failures,
    });
    return result.failures.length ? 2 : 0;
  } catch (error) {
    if (error instanceof UsageError) { console.error(error.message); return 64; }
    console.error(error instanceof Error ? error.message : "internal validation error"); return 70;
  }
}

...
Load [document-routing.md](modules/document-routing.md) when the repository exposes domain context, ADRs, nearest-README inheritance, or a multi-context map.
...
|---|---|---|
| repository/domain document route | [document-routing.md](modules/document-routing.md) | document declaration |
| meaning-based code candidates | [semantic-retrieval.md](modules/semantic-retrieval.md) | candidate |
| symbol definitions, references, diagnostics, or safe edit planning | [symbol-operations.md](modules/symbol-operations.md) | symbol candidate |
| cross-module or cross-language dependency candidates | [code-graph-impact.md](modules/code-graph-impact.md) | graph candidate |
| prior decisions, preferences, incidents, or session continuity | [project-memory.md](modules/project-memory.md) | memory hint |
...
```json
{
  "schema_version": "repo-agent-native-output/v2",
  "subject": {"root": ".", "commit": "<immutable-id>"},
  "routes": [],
  "invariants": [],
  "negative_invariants": [],
...
Run the bundled structural checker when changing this Skill:

```bash
python3 scripts/check_repo_agent_native.py --selftest
python3 scripts/check_repo_agent_native.py .
```
...
Exit `0` means the checker accepted its declared subject. Non-zero means the assertion failed. Never catch a failed assertion with `|| true`, rewrite `NOT_EXERCISED` as `PASS`, or keep retrying until a stochastic output happens to pass.

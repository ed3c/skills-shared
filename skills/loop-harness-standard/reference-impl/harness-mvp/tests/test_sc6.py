...
def test_sc6_budget_wins_when_budget_dup_and_max_iterations_all_hit():
    config = L3Config(max_iterations=2, max_tokens=2, duplicate_threshold=2, duplicate_window=4)
    state = L3State()

    first = l3_after_record(state, config, _record("r-1", result="same", tokens_used=1))
    second = l3_after_record(state, config, _record("r-2", result="same", tokens_used=2))
...
def test_sc6_duplicate_alone_triggers_dup_reason():
    ...
    first = l3_after_record(state, config, _record("r-1", result="same", tokens_used=1))
    second = l3_after_record(state, config, _record("r-2", result="same", tokens_used=1))

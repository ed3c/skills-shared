...
def _record(
    record_id: str, *, args: dict[str, object], result: str
) -> dict[str, object]:
...
def test_sc5_consecutive_duplicate_kills_on_threshold_step():
    config = L3Config(
        max_iterations=10, duplicate_threshold=3, duplicate_window=4, max_interleaving=3
    )
    state = L3State()

    decisions = [
        l3_after_record(
            state, config, _record(f"r-{index}", args={"slot": "A"}, result="same")
        )
...
def test_sc5_duplicate_window_smaller_than_max_interleaving_is_rejected():
    with pytest.raises(
        ValueError, match="duplicate_window must be >= max_interleaving"
    ):
...
def test_sc5_minimal_window_n_kills_at_threshold_but_leaks_for_n_plus_one():
    ...
    n_decisions = [
        l3_after_record(
            n_state, config, _record(f"n-{index}", args={"slot": slot}, result="same")
        )
...
def test_sc5_interleaved_duplicate_kills_on_threshold_step():
    config = L3Config(
        max_iterations=10, duplicate_threshold=2, duplicate_window=4, max_interleaving=4
    )
    state = L3State()
    decisions = []
    ...
    for index, slot in enumerate(["A", "B", "C", "A"], start=1):
        decisions.append(
            l3_after_record(
                state, config, _record(f"r-{index}", args={"slot": slot}, result="same")
            )
        )
...
def test_sc5_polling_with_changing_results_is_not_killed():
    ...
    decisions = [
        l3_after_record(
            state, config, _record(f"r-{index}", args={"slot": "poll"}, result=result)
        )
...
def test_sc5_retry_with_different_results_is_not_killed():
    ...
    decisions = [
        l3_after_record(
            state, config, _record(f"r-{index}", args={"attempt": 1}, result=result)
        )

...
|---|---|---|
| 2×2 host×driver 平行設定矩陣(Claude Code／Antigravity CLI 雙 host) | **不搬平行設定樹；改為 macro carrier × small driver 的觀測矩陣** | skill-bettor 現可由 Claude Code 或 Codex 承載 macro session，small driver 另選 `claude -p`/`agy`/`codex`。兩軸用同一組 B1-B8 owner 與 receipt 觀測，不各建 settings/skills 治理副本；驗證隔離看 author×judge 家族，不看 host 名稱。 |
...
若有人往本 skill 塞回 D 編號決策碼、antigravity 具體 commit 錨，或為每個 carrier 重建一套平行治理

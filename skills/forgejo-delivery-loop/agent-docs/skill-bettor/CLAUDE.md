...
本 repo **鏡像此模型之結構**(主權敘述除外,見末段):skill surface 已由 `.agents/skills/`
同名 symlink 提供 Codex 發現，但內容 canonical 與主權仍 Claude-first；完整 SSOT 暫由
...
## tier-dispatch(詳表 ARCHITECTURE.md §5、§5.1 交替、§5.2 降級梯子)
Opus 5=大迴圈編排(用途不限計畫);判官=fresh zero-context 且**與 author 跨家族優先**;
Sonnet=演化 author+eval agent(釘死,量尺永不換家族);Haiku/Flash 3.5=機械(禁 author 禁 verdict);
agy Pro 3.1=DR/跨家族複核(任何降級級別都只 findings 不 verdict)。
單家族＋agy=`degraded-independence` 必標記;單家族無 agy 只出 candidate。

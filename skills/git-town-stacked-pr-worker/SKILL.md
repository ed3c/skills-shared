...
## Owned gates and shared-body boundary

Run [`tests/run-all.sh`](tests/run-all.sh) before publishing changes to this Skill. It is the only route that reaches every gate this Skill owns — `check_stack_contract.py`, `check_fanout_contract.py`, `check_publication_boundary.py`, `check_repository_profile.py` and `assert_molecular_stack_index.py` — and a gate nothing runs is indistinguishable from a gate that was deleted.

...
CLOSURE_LAWS = tuple(f"CORE-LAW-{index:03d}" for index in range(9, 15))
...
def features(text: str) -> dict[str, Any]:
    ...
    return {"treatment_blob": blob(text), "task_gate": "scripts/assert_task_contract.py" in text,
            ...
            "closure_laws": all(law in text for law in CLOSURE_LAWS) and "completion-readiness" in lower and "do not substitute" in lower}

...
def score(arm: Arm) -> dict[str, bool]:
    ...
    # Issue #332: the six closure laws are bound into the core body itself, not
    # parked in a reference. Each clause is scored on what its law says, so a
    # bare heading, a law number, or a link to the reconciliation document
    # cannot earn the point.
    core_lower = (core or "").casefold()
    closure_laws = all((
        "start_dependencies" in core_lower and "completion_dependencies" in core_lower and "convergence owner" in core_lower,
        "repository reality readback" in core_lower and "reconcile in order" in core_lower,
        "closure matrix" in core_lower and "deterministic_run_observed" in core_lower and "live_substrate_observed" in core_lower,
        "closure lanes" in core_lower and "do not substitute" in core_lower and "lane equality is compared literally" in core_lower,
        "start-readiness" in core_lower and "completion-readiness" in core_lower and "cannot imply" in core_lower,
        "completion edge carries a receipt" in core_lower and "never inferred from the branch graph" in core_lower,
    ))
    ...
    return {
        ...
        "closure_laws_bound_in_core": closure_laws,
    }
...
def main() -> int:
    ...
    print("TECH-LEAD-AB-GREEN B0 regressions exposed; B1 restores reachability; B2 adds receipt-gated causal closure; B3 binds the six closure laws into the portable core and dominates every prior deterministic treatment; live model/provider A/B NOT_EXERCISED")
    return 0

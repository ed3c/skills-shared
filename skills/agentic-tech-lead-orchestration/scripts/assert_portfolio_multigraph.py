...
Salvaged from PR#562 (caedeb9), with #566 mandatory fix 2 applied: wave
scheduling is validated against G1 (start_dependencies) predecessors only.
"""
...
def verify(graph: dict[str, Any], schema: dict[str, Any] | None = None) -> None:
    ...
    # #566 mandatory fix 2: predecessors gating wave placement come from G1
    # (start_dependencies) only. G2 (completion_dependencies) already
    # produces its own G7 PUBLICATION_DEPENDENCY edge and must never be
    # promoted into a dispatch-order predecessor -- merging G1+G2 here (the
    # pre-#566 shape) is the START_DEPENDENCY_PROMOTED_TO_COMPLETION defect:
    # it wrongly rejects a valid graph where a completion-only child is
    # dispatched in the same wave as its parent.
    predecessors: dict[str, set[str]] = {node: set() for node in nodes}
    for edge in graph["graphs"]["G1"]:
...
def positive_fixture() -> dict[str, Any]:
    ...
    graph = {
        ...
        "epoch_digest": "1" * 64,
        "epoch_subject": {"main_commit": "a" * 40, "tree": "b" * 40},
        "nodes": [
...
def selftest(schema: dict[str, Any] | None) -> None:
    ...
    # #566 fix 2 regression (positive direction): a node with ONLY a
    # completion_dependency (G2, no G1 edge) on its parent must be allowed to
    # dispatch in the SAME wave as that parent. Pre-#566 this raised
    # "scheduled before predecessors" because G2 was wrongly merged into the
    # wave-gating predecessor set (START_DEPENDENCY_PROMOTED_TO_COMPLETION).
    completion_only_parallel = copy.deepcopy(base)
    completion_only_parallel["graphs"]["G1"] = []
    completion_only_parallel["ready_waves"] = [["A", "C"], ["B"]]
    completion_only_parallel = bind_digest(completion_only_parallel)
    verify(completion_only_parallel, schema)
    print("REFUSED-NOT START_DEPENDENCY_PROMOTED_TO_COMPLETION (completion-only child parallel-dispatched, correctly accepted)")

    print(f"PORTFOLIO-MULTIGRAPH-GREEN positives=2 mutations={len(mutations)}")

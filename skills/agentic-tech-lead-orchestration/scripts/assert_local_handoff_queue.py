...
def validate(queue: dict[str, Any]) -> list[str]:
    ...
    for index, item in enumerate(items):
        ...
        if not isinstance(commands, list) or not commands:
            errors.append(f"{item_id}: command lane missing")
        ...
        forbidden_promotions = receipt.get("forbidden_promotions", [])
...
def selftest(queue: dict[str, Any]) -> list[str]:
    ...
    def add(name: str, mutate, needle: str) -> None:
        cases.append((name, mutate, needle))
    ...
    add("missing command", lambda q: q["items"][0]["runtime_lane"].__setitem__("commands", []), "command lane missing")
    ...
    for name, mutate, needle in cases:
        ...
        if not any(needle.lower() in error.lower() for error in found):
...
def main() -> int:
    ...
    if errors:
        ...
        return 2
    if args.selftest:
        print("SELFTEST GREEN: Local Handoff Execution Queue controls")
    else:
        print(f"PASS: Local Handoff Execution Queue ({len(queue['items'])} item(s), active={queue['current']['active_item']})")
    return 0

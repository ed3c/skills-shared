#!/usr/bin/env python3
"""Fail closed on unresolved model aliases, egress, sandbox mismatch, or invalid role leases.

Salvaged from PR#562 (caedeb9), with `sandbox_mode` added to the dispatch
shape (target layout: "budgets/sandbox_mode/expected_output fields") and
checked against each role's fixed sandbox posture.
"""
...
ROLE_SANDBOX_MODE = {role: "read-only" for role in READ_ONLY_ROLES}
ROLE_SANDBOX_MODE["implementation-worker"] = "workspace-write"
...
def verify(packet: dict[str, Any], schema: dict[str, Any] | None = None) -> None:
    ...
    if packet["sandbox_mode"] != ROLE_SANDBOX_MODE[packet["role"]]:
        raise ValueError("SANDBOX_MODE_ROLE_MISMATCH")
...
def selftest(schema: dict[str, Any] | None) -> None:
    ...
    sandbox_mismatch = copy.deepcopy(base)
    sandbox_mismatch["sandbox_mode"] = "workspace-write"
    mutations.append(("SANDBOX_MODE_ROLE_MISMATCH", bind_digest(sandbox_mismatch)))

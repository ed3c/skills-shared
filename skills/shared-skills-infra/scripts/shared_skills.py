...
never in the versioned registry. Clone anywhere, run `install`, done.
  ...
  commands generate each project's `.claude/commands/<name>.md` forwarders
...
# --------------------------------------------------------------------------
# command forwarders: generated, never hand-maintained (#1 ruling 4)
# --------------------------------------------------------------------------
#
# A `.claude/commands/<name>.md` names one skill and one repository's surface
# path, so by #1's own test -- move it unchanged to another repository; is it
# still true? -- it is binding, not body. Binding that is mechanically derivable
# should be derived: five repositories hand-maintaining the same forwarder is
# five places for the same drift.
#
# The alias list lives in the registry beside the ruling that admitted the skill,
# because "delivery means github-delivery-loop here" is a ruling, not a path.

COMMAND_MARKER = "<!-- generated by shared_skills.py commands -->"
COMMAND_TEMPLATE = """---
...
載入並完整遵循 `.claude/skills/{skill}/SKILL.md`，將 `$ARGUMENTS` 原樣交給它處理。
本檔不得自行定義分派、worktree、交付或驗證程序：那些只存在於 canonical skill，
在此複述一份就是無聲的第二真源。要改別名，改 registry.json 的 `commands`。
"""


def render_command(name: str, skill: str) -> str:
    description = (
        f"{skill} 的本 repo 命令入口；程序只存在 canonical skill"
        if name == skill
        else f"{skill} 的相容別名；程序只存在 canonical skill"
    )
    return COMMAND_TEMPLATE.format(
        description=description, marker=COMMAND_MARKER, skill=skill
    )


def command_names(registry: dict[str, Any]) -> dict[str, str]:
    """command name -> skill. Two skills claiming one name is unresolvable."""
    mapping: dict[str, str] = {}
    for item in registry["shared"]:
        skill = item["name"]
        aliases = item.get("commands", [])
        if not isinstance(aliases, list):
            raise SharedSkillsError(f"{skill}: registry `commands` must be a list")
        for name in [skill, *aliases]:
            if not isinstance(name, str) or not name or any(
                char not in "abcdefghijklmnopqrstuvwxyz0123456789-" for char in name
            ):
                raise SharedSkillsError(
                    f"{skill}: command name {name!r} must be lowercase letters, "
                    "digits and hyphens -- it becomes a filename and a slash command"
                )
            if name in mapping and mapping[name] != skill:
                raise SharedSkillsError(
                    f"command name '{name}' is claimed by both {mapping[name]} and "
                    f"{skill}; a slash command that resolves to two skills resolves "
                    "to neither"
                )
            mapping[name] = skill
    return mapping
...
def commands(registry: dict[str, Any], sites: Sites, apply: bool) -> int:
    """Write every project's command forwarders from the registry.

    Only files this generator wrote are ever rewritten or removed; the marker is
    what says so. A hand-maintained file at a generated name is SURFACE, not
    FAIL: it is the un-migrated state #1 describes, and it usually holds host
    binding that has to move to `.skill-bindings/` before the name is free.
    Overwriting it would destroy exactly the content the migration exists to
    preserve, and failing on it would block every other repository on it.
    """
    mapping = command_names(registry)
    if not sites.projects:
        print("NOTHING-TO-DO: no project is wired; `install --project ...` first")
    ...
    for project in sites.projects:
        if not project.is_dir():
            continue
        root = project / ".claude" / "commands"
        for name, skill in sorted(mapping.items()):
            path = root / f"{name}.md"
            desired = render_command(name, skill)
            if path.is_file():
                current = path.read_text(encoding="utf-8", errors="replace")
                if COMMAND_MARKER not in current:
                    foreign.append(path)
                    continue
                if current == desired:
                    unchanged += 1
        ...
        for path in sorted(root.glob("*.md")):
            if path.stem in mapping:
                continue
            if COMMAND_MARKER not in path.read_text(encoding="utf-8", errors="replace"):
                continue  # somebody else's file, under a name we do not claim
            removed.append(path)
            if apply:
                path.unlink()
    ...
    for path in written:
        print(f"{verb} {path}")
    for path in removed:
        print(f"{'REMOVED' if apply else 'WOULD-REMOVE'} {path}  (no longer in registry)")
    print(
        f"COMMANDS {len(written)} to write, {len(removed)} stale, {unchanged} already current"
        + ("" if apply else "  -- add --apply to execute")
    )
    if foreign:
        # Reported after the counts so it cannot be mistaken for the summary.
        print(
            f"SURFACE {len(foreign)} hand-maintained command file(s) hold a generated "
            "name; move their host binding to .skill-bindings/<skill>/ and delete them:"
        )
        for path in foreign:
            print(f"  {path}")
        return NOTHING_TO_DO
...
def _parser() -> argparse.ArgumentParser:
    ...
    with_paths(
        subcommands.add_parser("install", help="wire this clone to a machine and link")
    )
    with_paths(
        subcommands.add_parser("report", help="classify every skill name; decision queue")
    )
    with_paths(
        subcommands.add_parser("check", help="zero-network gate over the rulings")
    )
    link_parser = with_paths(
        subcommands.add_parser("link", help="materialize a skill's symlinks")
    )
    link_parser.add_argument("name")
    adopt_parser = with_paths(
        subcommands.add_parser("adopt", help="move a copy in and register")
    )
    ...
    command_parser = with_paths(
        subcommands.add_parser(
            "commands", help="generate each project's .claude/commands forwarders"
        )
    )
    command_parser.add_argument(
        "--apply", action="store_true", help="write; without it, nothing is touched"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "commands":
            return commands(registry, sites, args.apply)
        return adopt(
            registry,
            sites,
            args.name,

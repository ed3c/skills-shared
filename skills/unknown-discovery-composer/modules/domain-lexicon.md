...
| Retarget fact | Platform difference, missing upstream mechanism, historical mapping | `modules/retarget-map.md` | Keep history out of the main workflow. |
...
| `repo-wiki-converge gap` | Opus-grade prose repo wiki convergence from upstream has no local equivalent unless built later. | Surface as missing capability; do not silently replace with `repo-agent-native`. | If route needs prose convergence, mark `human_required`. |

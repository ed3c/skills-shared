...
| `DEC-DELIVERY-12` | One primary workflow remains the publication and billing authority, while policy-declared auxiliary workflow/job pairs may contribute exact-head repair feedback. Capture requires the complete declared set and reduces all actionable failures to one deterministic batch identity; missing, incomplete, stale, or rerun evidence blocks. | `ci_workflow_policy.py`, `github_actions_snapshot.py`, `ci_publish_gate.py`, issue #261 |

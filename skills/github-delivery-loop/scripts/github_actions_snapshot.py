...
def observation_from_transport(raw:dict[str,Any])->dict[str,Any]:
    ...
    if pulls:
        ...
        if len(entries)<=index or entries[index]["argv"][2]!=endpoint:raise SnapshotError("transport branch-ref argv mismatch")
        ...
        elif "404" not in entry["stderr"] and "Not Found" not in entry["stderr"]:raise CaptureError(f"gh api failed for {endpoint}")
    if index!=len(entries):raise SnapshotError("transport contains unconsumed provider calls")
    return {"schema":OBSERVATION_SCHEMA,"repository":rv,"branch":{"name":branch_name,"head_sha":head},"pull_requests":pulls,"check_runs":checks,"captured_at":timestamp(raw["captured_at"],"transport.captured_at")}
...
def main(argv:list[str]|None=None)->int:
    p=argparse.ArgumentParser();p.add_argument("--selftest",action="store_true");subs=p.add_subparsers(dest="cmd");r=subs.add_parser("replay");r.add_argument("--observation",type=Path,required=True);r.add_argument("--check-name",required=True);r.add_argument("--output",type=Path,required=True);t=subs.add_parser("replay-transport");t.add_argument("--transport",type=Path,required=True);t.add_argument("--observation-output",type=Path,required=True);t.add_argument("--output",type=Path,required=True);c=subs.add_parser("capture");c.add_argument("--repository",required=True);c.add_argument("--branch",required=True);c.add_argument("--check-name",required=True);c.add_argument("--timeout-seconds",type=int,default=30);c.add_argument("--transport-output",type=Path,required=True);c.add_argument("--observation-output",type=Path);c.add_argument("--output",type=Path,required=True);a=p.parse_args(argv)
    if a.selftest:
    ...
    try:
        ...
        if a.cmd=="replay-transport":
            raw=load(a.transport,"transport");o=observation_from_transport(raw);v=build(o,raw["check_name"]);atomic(a.observation_output.resolve(),o);atomic(a.output.resolve(),v);print(f"WROTE {a.output.resolve()}");return 0
        if a.cmd=="capture":
            if a.timeout_seconds<1:raise CaptureError("timeout must be positive")
            raw=capture_transport(a.repository,a.branch,a.check_name,a.timeout_seconds);o=observation_from_transport(raw);v=build(o,a.check_name)

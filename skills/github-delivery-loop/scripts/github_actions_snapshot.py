...
def build(raw:dict[str,Any],check_name:str)->dict[str,Any]:
    ...
    if not pulls:
        ...
        return {"schema":SNAPSHOT_SCHEMA,"repository":v["repository"],"pull_request":{"number":None,"state":"absent","head_sha":None,"last_published_sha":None,"last_published_at":None,"feedback":None},"actions":{"circuit":"closed","observed_at":None,"blocker":None,"latest_check":None}}
    p=pulls[0]
    ...
    return {"schema":SNAPSHOT_SCHEMA,"repository":v["repository"],"pull_request":{"number":p["number"],"state":"draft" if p["draft"] else "ready","head_sha":p["head_sha"],"last_published_sha":p["head_sha"],"last_published_at":p["updated_at"],"feedback":feedback},"actions":actions}

...
5. `references/IMPLEMENTATION_PREFLIGHT.md` — current Tech Lead stage boundary and terminal work packets.
6. `references/NEGATIVE_CONTROL_MATRIX.md` — required deterministic controls before semantic closure.
7. `../spatial-loop-systems-engineering/references/intent-case-proof-graph.md` — canonical ICPG semantics.
8. `../agentic-tech-lead-orchestration/README.md` — case ownership, task DAG, leases and convergence.
9. `../git-town-stacked-pr-worker/README.md` — exact Stack relation semantics and Molecular Stack rules.
10. `references/TRACE_GRAPH_CONTRACT.md` — node classes, edge vocabulary, authority/freshness/evidence rules.
11. `references/SYSTEM_PROMPT_V7_2_DELTA.md` — historical/maintenance delta explaining what v7.2 adds over v7.1; do not require a fresh Agent to compose the prompt manually from this file.
12. exact issue/PR/ref/SHA/file/workflow/receipt subjects before any current-state claim.
...
The prompt-design stage is currently complete as a design artifact. Do not reopen prompt design merely for wording changes; the next admitted work is deterministic checker/tests unless an executable control exposes a semantic defect.
...
- a second decision-active full v7.2 prompt is being created;
- a checker failure is being hidden by documentation-only changes.

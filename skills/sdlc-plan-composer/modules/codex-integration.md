...
> 本檔內容直接參照上游同日（2026-07-17）的等價研究（Backend 2 段；上游模組的絕對路徑與 repo
> 身分記在 `.skill-bindings/sdlc-plan-composer/`），因為官方 plugin 是 scope=user 全局安裝，同一台
> 機器同一使用者，上游與宿主共用同一份查證結果。
...
code-review，無 code-branch」是刻意設計邊界，不為塞新功能破壞它。宿主沒有
`repo-fullstack-debugger` 這類 skill（上游特有），故 `codex:codex-rescue` 與既有診斷 skill 的

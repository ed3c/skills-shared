# Module: mattpocock 全局 atomic skill 盤點(2026-08-13 對上游 84fdeff 同步後)
...
> 2026-08-13 上游同步:`design-an-interface`／`qa`／`request-refactor-plan`／`ubiquitous-language`／
> `edit-article`／`obsidian-vault` 已被上游移除(無接手者,本機留檔於 `~/.agents/skills/_archive/`);
> `to-prd`→`to-spec`、`to-issues`→`to-tickets`、`writing-great-skills`→`writing-for-agents`。
> 尚未落點的新上游 skill:`wizard`、`to-questionnaire`、`wait-what`、`setup-ts-deep-modules`、`ask-matt`。
...
| to-spec | S2 | 作者(不訪談,只綜合已知) | - | - |
| to-tickets | S2 | 作者+對話(tracer-bullet 切片;quiz 段) | - | quiz 段是 |

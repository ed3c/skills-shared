...
description: |
  ...
  造或改 skill（skill-authoring）。完整 agy／Opus／openwiki 執行經驗與 5 條收斂原則在 modules/。
---
...
>   - CREATE pass = `../../../kb-ingest/repo-wiki.workflow.md`（openwiki-distilled，深理解 workflow）。
>   - REFINE pass 指令 = `../../../kb-ingest/refine.prompt.md`；JUDGE 程序/rubric = `../../../kb-ingest/judge.prompt.md`。
>   - T0 pre-verifier = `../../../kb-ingest/verify-claims.sh`（機械檢 anchor/quote/frontmatter/Why-tag,FAIL 零判官成本退回;agy-pass.sh 每 pass 後自動跑）；成本帳本 = `../../../kb-ingest/engine-baseline.md`。
>   - Gemini 作者 runner（Claude 或人，從 host project CWD）= `../../../kb-ingest/agy-pass.sh` `<create|refine> <TARGET> <OUT> <GEMINI_MODEL>`。
>   - 缺口清單 SSOT = `<OUT>/_judge/gaps-latest.md`（Opus 每輪寫，作者每輪讀）。
>   - 進 KB = `python3 -m indexing.ingest_repodoc_cli <OUT>/repo_wiki/<slug>/`。
>   - 遷移完整性檢查 = `../../../kb-ingest/check_repo_wiki_converge.py`；它保護本 skill 不退化成外部 antigravity 路徑引用。
...
## When to Use
- 有一個**可讀 repo**（本地 clone）要產 Opus 級理解 wiki 並進 skill-bettor RepoDoc lane。
...
- ❌ 造／改 skill 或 skill 規範 → `skill-authoring（../skill-authoring/SKILL.md）`。
...
## 確定性程序（一鍵收斂流程）
0. **本地遷移 preflight**：在 skill-bettor root 跑 `python3 kb-ingest/check_repo_wiki_converge.py`。任何缺檔、舊絕對路徑、frontmatter provenance 漂移、RepoDoc ingest dry-run 失敗，都先修本地基座，不進作者輪。
1. **Clone 全歷史 + 定位置**：`bash kb-ingest/setup-repo.sh <repo_name> <clone_url>` 會 clone 到 `/Users/neon/skill-bettor/repo/<repo_name>/<repo_name>`（絕不 `--depth 1`；TARGET＝該 clone，basename＝slug）。**OUT＝`/Users/neon/skill-bettor/repo/<repo_name>/`**（durable，**絕不放 /tmp**；產物 `repo_wiki/`、`invariants/`、`_judge/` 都落此。`/repo/` 已 gitignore）。
...
3. **JUDGE（我，Opus session）**：step 0 先看 `_judge/preverify-latest.md`（agy-pass.sh 已自動跑 verify-claims.sh;FAIL＝直接用該報告當 gap list 退回,不燒判官 token 重推導）→ 照 `../../../kb-ingest/judge.prompt.md` 只裁機械檢不了的（覆蓋完整性/機制忠實度/slug/回歸 diff）—— 對 TARGET+git
...
5. **收斂（判官寫 CONVERGED=true）→ INGEST**：先跑 `python3 -m indexing.ingest_repodoc_cli <OUT>/repo_wiki/<slug>/ --dry-run`，再跑 `python3 -m indexing.ingest_repodoc_cli <OUT>/repo_wiki/<slug>/` 寫入 skill-bettor `.cache/kg/graph.json`。只有 `--embed` 需要 chromadb/Ollama；基本 RepoDoc graph ingest 不需要向量庫。

## Stateful Workflow（不要壓成單 prompt）
| Node | Actor | Input | Output | Conditional Edge |
|---|---|---|---|---|
| S0 local-preflight | Codex/Claude | `.claude/skills/repo-wiki-converge`, `kb-ingest`, `indexing` | migration report | fail → repair local port |
| S1 source-setup | setup-repo.sh | clone URL/local path | full-history TARGET + OUT | shallow → fetch/full-clone repair |
| S2 create | Gemini via agy-pass.sh | TARGET + repo-wiki.workflow.md | `repo_wiki/<slug>/` + preverify | preverify FAIL → S4 refine with report |
| S3 judge | Opus session | TARGET + wiki + preverify | `_judge/gaps-latest.md` | `CONVERGED=false` → S4 refine; true → S5 ingest |
| S4 refine | Gemini via agy-pass.sh | gaps-latest + wiki | updated wiki + snapshot | regression → S3 judge with diff emphasis |
| S5 ingest | indexing.ingest_repodoc_cli | converged wiki | RepoDoc/Library/Concept graph | dry-run FAIL → S4/S0 depending on cause |
| S6 handoff | Opus session | graph report + gaps history | L1 completion note / L2 scope seed | need invariants → repo-agent-native |

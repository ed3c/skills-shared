...
**分層真相（別把「wiki 收斂」當「repo 掌握」；完整階梯 + handoff SSOT → `../../../kb-ingest/mastery-ladder.md`）**：
...
## 8. 實踐與問題修復（DDR-20260707 新增）
### 8.1 Ingest 直譯器與環境依賴
* **根因**：Ingest 腳本 `indexing/ingest_repodoc_cli.py` 內部會 import `chromadb` 等套件。若直接使用系統預設的 `python3`，會因為套件缺失導致 Ingest 崩潰；而依賴通常裝在某個 workspace 的虛擬環境裡，不在系統 python。此外 Python 解析本地 `indexing` 模組時需要 `PYTHONPATH` 指到 repo 根，否則 import 路徑解不開。
* **處置**：執行 Ingest 時顯式指定裝了依賴的那個 `python3`，並前置 `PYTHONPATH=<repo 根>`。兩個具體值因機器與 repo 而異，寫在該 repo 的 `.skill-bindings/repo-wiki-converge/`，不寫死在共用 body。

### 8.2 Monorepo 子目錄 Git 版本判定
* **根因**：原有的 `agy-pass.sh` 僅依賴 `[ -d "$TARGET/.git" ]` 判定是否為可執行的 git 目錄。然而當 TARGET 為 monorepo 的子目錄（如 `ixsecurity/api-service`）時，該目錄本身沒有獨立的 `.git`，但它依然是在上層 Git 的版本控制下。
* **處置**：判定機制升級為 `{ [ -d "$TARGET/.git" ] || git -C "$TARGET" rev-parse --is-inside-work-tree; }`，使其相容 monorepo 子專案的 git commit/sha 解析。

### 8.3 大型專案與多頁面 Judge 超時
* **根因**：當專案規模擴大（Wiki 多達 15–20 頁，帶有數十個 quote/anchor）時，`agy` 判官需要大量時間逐一透過 `git grep` 與 `git show` 進行語意對照與事實查實。`agy` 預設的 5 分鐘 timeout 會導致命令中途被強制砍掉，呈現 silent no-op。
* **處置**：於 `agy` 判官呼叫中顯式加入 `--print-timeout 30m`，為大型專案釋放充足時間。

### 8.4 0 頁面生成與機械過關漏洞
* **根因**：當 Author 模型因 API 配額限制或 sandbox 掛載路徑錯誤而未能產出任何 wiki 頁面時（`pages=0`），`verify-claims.sh` 的機械檢驗器因「無程式碼故無錯誤」而判定 `verdict=PASS`。隨後 Judge 流程在無 gaps 的狀態下陷入 Fatal 迴圈。
* **處置**：機械驗證應前置確認 `pages > 0`，若為 0 則應直接阻斷為 `FAIL`。

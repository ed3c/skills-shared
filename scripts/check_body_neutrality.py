#!/usr/bin/env python3
"""Compatibility entrypoint for the canonical body-neutrality ratchet.
...
CANONICAL = ROOT / "skills" / "shared-skills-infra" / "scripts" / "check_body_neutrality.py"


def main(argv: list[str] | None = None) -> int:
    args = list(sys.argv[1:] if argv is None else argv)
    if not CANONICAL.is_file():
        print(f"FAIL canonical body-neutrality ratchet missing: {CANONICAL}", file=sys.stderr)
        return 2

...
├── run.sh                            # 2  調度入口 + driver switch(claude | agy | codex;單發 dispatch)
...
├── ROUTES.md                         # 4  capability catalog:agents,skills,validators,packet routes
├── modules/exchange-formats.md        # 5  typed cross-loop wire protocol;實例化時必填 schema/validator
...
├── .githooks/                        # B2/B3 commit lifecycle and authorization policy
├── .claude/skills/                   # B4/B5 capability catalog and specialized modules
├── scripts/
│   ├── check_all_skills.py         # B6 layered repo gate
│   ├── check_sdlc_plan_package.py  # B6/B7 typed plan validation
│   └── execute_sdlc_plan.py        # B8 digest-bound node executor
├── docs/plans/                      # B7/B8 intent, topology, evolution and receipts
└── loop_wiki/
    ├── engine.sh                     # bounded dispatch→verify→stop-loss→Human edge
    ├── _template/                    # paired small-loop B1-B8 seed
    └── <op-slug>/                    # instantiated small loop
```
...
`ROUTES.md` 是 B4 capability catalog：列 macro-loop agent/skill、small-loop 本地
`.agents/agents/*`/`.agents/skills/*` endpoint、validator 與 packet route，但不持有某次執行 DAG。
`modules/exchange-formats.md` 是 B5 typed specialization seam：schema、identity 與 validator 必須在實例化時
落地；物理交付觸發可落 `trigger.sh`,
...
per-op driver 四態(`run.sh` switch):
...
3. **對話內 subagent**(輕量;無沙盒執行體;prune 等輕量 op 適用):由大迴圈主 session 以 Agent tool
   ...
   subagent 判官;跨家族天然隔離)。
4. **`codex`**(OpenAI GPT，第三家族，2026-07-17 新增；經正式 Codex runtime，禁手刻未受管載具)：
   不依賴 `CLAUDE.md`/`AGENTS.md` 檔名自動載入；dispatch 必明確指示先讀沙盒 `AGENTS.md`(若存在)
   取得治理規範。隔離按 **author×judge** 家族判定：Codex author 對 Claude judge 是跨家族；Codex
   author 對 Codex judge 不是，必須 fresh zero-context。Macro carrier 是 Claude Code 或 Codex 都不改此判準。詳細用法/`--model`/`--effort`
...
[dual-loop-eight-base.md](dual-loop-eight-base.md) 與 [retarget-map.md](retarget-map.md)。

...
>   - DR 報告 → 保真 Markdown = `gemini-deep-research-extract`（skill-bettor 未隨本次複製；若本地不存在，S3 抽取必須標 `external_engine_required`，不可宣稱本 repo 已可直接執行）。
...
>   - skill-bettor 本地新增可跑 seed = [guided-conversation-observation](modules/guided-conversation-observation.md) + [production-guidance-hardening](modules/production-guidance-hardening.md) + `loop_wiki/evolve-unknown-discovery-plan-truth/templates/gemini-conversation-research/` golden cases/evals/ROUTES contract。
...
- ❌ 只要把一份**已生成**的 DR 報告抽成保真 md → `gemini-deep-research-extract`（本 skill 的 S3 抽取只是路由；skill-bettor 未安裝時必標 `external_engine_required`）。
...
| **S3 DEEP** | DR 投遞 + 抽取 | **複用外部 `automate.js --dr-once`(runDrOnce) + `gemini-deep-research-extract`**，skill-bettor 未安裝/未接線時標 `external_engine_required`，**不重造** |
...
Gemini 回應後可能出現非固定複製/分享 UI 的 contextual suggestion buttons，例如「是」、方向選項、比較選項、追問選項。這些按鈕是對話語義事件，必須按照上下文順序觀察、分類、觸發、抽取 auto prompt/auto answer，並把被簡化資訊、遺失資訊、遺失 Domain 專有名詞寫成 state。程序與 schema 見 [modules/guided-conversation-observation.md](modules/guided-conversation-observation.md)；小迴圈模板與 golden behavior eval 見 `loop_wiki/evolve-unknown-discovery-plan-truth/templates/gemini-conversation-research/`。

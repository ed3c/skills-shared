...
| **S3** DR 投遞+抽取（`submit-dr-prompt.ts` + `extract-dr-report.ts`，northstar 版） | **拿掉 northstar 版** → 接 `automate.js` `runDrOnce`(2287)/`runGeminiDeepResearch*`(1364/1383) + [`gemini-deep-research-extract`](../../gemini-deep-research-extract/SKILL.md) | 見 §3 **DR-reuse 命門**：northstar 版是 antigravity 引擎的**滯後複製**，實證間歇卡。 |
...
- **S3 投遞** → `automate.js` `runDrOnce(promptFile, outFile)`（2287）或 `runGeminiDeepResearch*`。批次序列走「脫鉤才投下一個」（DR 槽只在 init ~2-3min 被佔）。
- **S3 抽取** → [`gemini-deep-research-extract`](../../gemini-deep-research-extract/SKILL.md)（HTML→turndown 保真、`[cite:N]`、來源只取「用來生成報告」段）。
...
- antigravity 慣例：[`antigravity-skill-authoring`](../../antigravity-skill-authoring/SKILL.md)、[`judge-loop-chooser/modules/retarget-map.md`](../../judge-loop-chooser/modules/retarget-map.md)（同型 port 先例）、[`AGENTS.md`](../../../../AGENTS.md)。

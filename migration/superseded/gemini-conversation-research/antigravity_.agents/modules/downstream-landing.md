...
> D2 產出的是「架構設計」，本節是把它**落成一個有守門測試的 skill** 的步驟序列（actuate 委派 [antigravity-skill-authoring](../../antigravity-skill-authoring/SKILL.md)，**非在此重造 skill 規範**）。錨＝cc-20260711 真跑產物 `ios-realdevice-automation`（絕對路徑 `/Users/neon/ix-agy/.agents/skills/ios-realdevice-automation/`，`tests/run-all.sh` 全綠）。
...
5. **LAND（actuate → antigravity-skill-authoring）**：canonical skill 落地——

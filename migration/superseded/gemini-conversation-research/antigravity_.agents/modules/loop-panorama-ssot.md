...
|------|------|------|
| **S0 EXTRACT** | 抽取的對話文本 `gemini_research/<slug>-conversation.md` | 抽取規則 SSOT 在 [conversation-pipeline.md §S0](conversation-pipeline.md)；UI selector 真相在 `automate.js`，別在此改 |
...
| **S3 DEEP** | Gemini Deep Research 報告 | **引擎 = `automate.js runDrOnce`，不重造**（§5 命門）；抽取委派 [gemini-deep-research-extract](../../gemini-deep-research-extract/SKILL.md) |
...
| **DR 抽取 / 完成偵測 prompt·邏輯** | [gemini-deep-research-extract](../../gemini-deep-research-extract/SKILL.md) + `automate.js` | HTML→turndown 保真 / `[cite:N]` / 完成判據 | 逐字 SSOT 在 sibling skill + 引擎，**別在 gcr 留簡化副本** |

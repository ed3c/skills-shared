...
> **2026-07-27 current override**：production dispatch 已由 Claude `Agent`/`Workflow`＋
> `codex:codex-rescue` carrier 改為當前 Codex 主 session 統籌、Codex 原生 worktree execution、fresh GPT
> findings-only judge 與 human LAND。舊段落保留為 lineage，不是現行路由。現行 SSOT 是 `../SKILL.md`
> M4/G4/V5/V6。Claude→GPT judge/author 仍是 `candidate`，必須固定 corpus 並列校準；不可由文字 retarget
> 宣稱 `technical_equivalent`。
...
- `superpowers:*`：`~/.claude/plugins/marketplaces/superpowers-marketplace`（marketplace 註冊目錄）+
  ...
  的持有狀態。
- 同批移植 sibling：撰寫本檔當下，`ls /Users/neon/skill-bettor/.claude/skills/` 只有

...
> **2026-07-27 current override**：下方 Claude/Fable/Opus/Sonnet 四角色表只保留為歷史 lineage，不是
> production dispatch。現行角色為：Opus 5 主 session＝高推理編排（用途不限計畫／判官）；
> mechanical script＝決定性事實；Codex execution session＝單 slice implementation；fresh、non-forked、
> read-only judge＝findings-only semantic judge；human＝唯一 LAND。author 與 judge 預設跨家族，
> 不是固定綁 GPT judge。每個 dispatch 必先讀 matching
> `execution-topology.json` node 與
> `loop_wiki/evolve-unknown-discovery-plan-truth/modules/semantic-truth-context.md`。無 native-worktree receipt、
> writer-set/resource namespace、cancellation/partial-result contract 時，write-capable delegation fail closed。
> 修復由 `evolution-ledger.json` 限三代；duplicate digest/no progress 直接 `stop-human`。

### 獨立性降級梯子（receipt 必須機器可讀）

1. `cross-family`：`author_family != judge_family`，可支撐 promotion。
2. `degraded-independence`：只剩單一家族時，judge 必須 fresh zero-context、不同 session，並有 agy
   `findings-only` correlated-error 探針；高風險項停人閘。
3. `candidate`：單一家族且 agy 不可用，只產候選，禁止自動 promotion。

禁止 silent fallback；receipt 必填 `author_family`、`judge_family`、`independence_level`、
`agy_findings_present`、`agy_role=findings-only`。agy 永不取得 verdict 權。
...
> Historical only；現行分工以上方 override 與 `../SKILL.md` M4/G4/V5/V6 為準。

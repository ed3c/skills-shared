...
description: |
  ...
  （design-an-interface + 決策記錄稀疏三條件，非編號 ADR）→ S4 子代理分治計劃（superpowers）→
...
- ❌ **重寫六階段 atomic skill**：`grill-with-docs` / `to-prd` / `to-issues` / `tdd` / `diagnose` /
  ...
  只序列委派，不複製。
- ❌ **取代 `superpowers:writing-plans`**：writing-plans 是通用計劃骨架；本 skill 是**在其之上**疊加
  SDLC 階段語義 + atomic-skill 路由的薄層（單階段/需求已很清楚時，直接用 writing-plans 更省）。
- ❌ **單步 / 探索性需求未成形**：需求還在霧裡，先走 `unknown-discovery-composer`（U0-U1 收斂到「能寫
  計劃」）或 `superpowers:brainstorming`，收斂後才用本 skill。
...
| **S4 子代理分治計劃** | `superpowers:subagent-driven-development` / `superpowers:dispatching-parallel-agents` | 每階段 plan 細節（子代理 full-detail 撰寫） | 主會話只持指針；**不簡化**；遵守 BS #420 三角互斥（見下） |
...
> ⚠️ `superpowers:*` 為 Claude Code plugin 級 skill，理論上跨 project 全局可用；**未在 antigravity
> session 現場驗證**——若失效，S4 分治退回人工用 `to-prd` + 手寫子任務清單。
...
**S5 優化迴圈**（northstar 原委派 `autoresearch-composer`）：**antigravity 無對應基座，誠實留白**——若
某 task 本質是「有界 modify→verify→keep/discard 對某可量測指標迭代」，直接在該 `NN-slice.md` 的執行
契約段 inline 寫 Goal/Scope/Metric/Guard/Iterations，不委派任何 composer skill（無則不造）。
...
- **與 `unknown-discovery-composer` 的邊界**：該 skill U1 出口在需求已收斂到「能寫計劃」後委派本
  skill（若任務多階段、需要 SDLC 級紀律）；需求單一或簡單則仍走 `superpowers:writing-plans`。

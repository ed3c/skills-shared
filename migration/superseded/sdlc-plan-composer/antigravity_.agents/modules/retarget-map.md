...
> ADR）。協議骨架本身乾淨映；治理基座**無/未-wire 已拿掉並記錄**。`autoresearch-composer` 優化迴圈
> 委派原記為缺口，2026-07-17 已港補齊本地 fork（見下表該行）。
...
**2026-07-17 retarget（S4）**：`superpowers:subagent-driven-development` / `superpowers:dispatching-parallel-agents`
原本帶著「Claude Code plugin 級 skill，理論上同樣全局可用（未在 antigravity session 現場驗證）」的保留
意見——查證 `installed_plugins.json` 後確認 superpowers 從未在 antigravity 掛號（只掛在 `local_stack`/
`TrueMe_Android`/`northstar` 三個其他專案），拿掉理論性 fallback。查了 `gh api
repos/mattpocock/skills/git/trees/main`（mattpocock/skills 官方完整目錄），確認裡面**沒有**任何
「fresh-subagent-per-task 分治派工」技術——`implement` skill 本身是單一連續 session（用 tdd+code-review），
不是分治機制，這是架構哲學差異非命名差異。真正對應這個技術的是 Claude Code 自己的原生 `Agent`/`Workflow`
工具（superpowers 那個 skill 原本教的就是怎麼手動用 fresh-subagent-per-task，現在是 harness 一級能力）。
2026-07-17 進一步擴充：S4 分治派工非只原生工具，補上 `codex exec`（OpenAI GPT-5.5/5.4/5.4-mini，external-verify
查證用法）與既有 `agy`（Gemini Pro/Flash）兩個 backend，三者人工三選一、非自動觸發——完整機制 →
[modules/multi-model-subagent-dispatch.md](multi-model-subagent-dispatch.md)。原始 superpowers 版文字
封存於 `.archive/superpowers-skills-v6.1.1/{subagent-driven-development,dispatching-parallel-agents}/`。
...
| S5「S5 優化迴圈委派 `autoresearch-composer`」 | **2026-07-17 更新：已港補齊**——`.agents/skills/autoresearch-composer/SKILL.md` 本地 fork 落地，S5 該行改指回它 | 原先誤判為「無對應 `/autoresearch:*` 命令基座」；2026-07-17 現場 `ls ~/.claude/commands/autoresearch/` 才確認外部 autoresearch 引擎其實是**全局裝**（非 project-scoped），一直都可用，缺的只是 northstar 那層路由/讓位/契約注入邏輯——今日把那層 port 進來，逐機制映射見 `autoresearch-composer/modules/retarget-map.md`（本表不重複，指針即可）。 |
...
- **有本地基座可換的換**（比 northstar 原版更乾淨）：`repo-agent-native`（S-1）、`judge-loop-chooser`
  （S5）、`autoresearch-composer`（S5 優化迴圈，2026-07-17 港補齊），從「跨 repo 引用 northstar」換成
  「引用 antigravity 本地 fork」；連帶 artifact schema 從 northstar 的多檔案 YAML 改成 antigravity 的
  單頁多條目。
- **無基座、真實能力差距、誠實拿掉並記錄**（本表核心）：`hallucination_audit.py` 機器閘（降 SURFACE）、
  ...
  encd-infrastructure-hub/skill-cycle 整合接點（無此編排層）、
...
- **2026-07-17 更新**：`superpowers:subagent-driven-development` / `superpowers:dispatching-parallel-agents` /
  `superpowers:writing-plans` / `superpowers:brainstorming` 四處理論性引註**皆已拿掉**——
  `installed_plugins.json` 查證 superpowers 從未在 antigravity 掛號。S4 改指 Claude Code 原生
  `Agent`/`Workflow` 工具（+ `codex exec` + `agy` 兩個額外 backend，見
  modules/multi-model-subagent-dispatch.md）；`writing-plans`/`brainstorming` 分別改指 `to-prd`+`implement`
  / `grilling`（mattpocock 全局 skill，已於 `~/.agents/skills/` 現場確認存在）。

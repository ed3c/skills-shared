...
description: |
  ...
  每段路由結果由人 admit。實作前收斂到「能寫計劃」→ 多階段任務交棒 `sdlc-plan-composer`，
  ...
  2026-07-17 更新：單階段出口不再指理論性 `superpowers:writing-plans`（該 plugin 從未在本專案掛號），
  改指 mattpocock 真實技術等價物 `to-prd`+`implement`；見下方與 retarget-map。）
---
...
## Not For
- ❌ **需求已成形、可直接寫計劃**：多階段任務且要 SDLC 級紀律 → `sdlc-plan-composer`（antigravity 本地 fork，S-1..S5 序列委派）；單階段/需求已很清楚 → `to-prd`+`implement`。
...
> **三態 grounding（指針，2026-07-19 fold）**：KK 格每條「known」與 U1 收斂出的每條結論，交出口 gate 前
> 先分 repo-internal／external／[推論] 三態——定義與升級路徑見 `sdlc-plan-composer/SKILL.md` (g)
> 外部-currency lane，本檔不重抄。
...
> **派工紀律（指針，2026-07-19 fold）**：上表凡涉及檢索／comprehension 派工的行（`research`／
> `repo-agent-native`／`gemini-conversation-research`／`zoom-out`）按
> `sdlc-plan-composer/modules/multi-model-subagent-dispatch.md` 原則五執行——尤其 (2) handoff 傳
> `file:line` 指針非散文摘要、(3) agy niche＝外部-currency＋跨家族 truth-check（例行 repo 檢索行改指
> `grepai`/`serena`，不燒 agy）。

**出口 gate**：四象限各格「已收斂或顯式接受殘留」→ 人 admit → **[推論] 態條目不可標「已收斂」過
gate**——要嘛升級到 repo-internal／external（走本表對應路由），要嘛顯式標「接受殘留（[推論]，風險
自負）」；過 gate 的結論帶三態標注隨 handoff 交棒（sdlc S-1/(g) 對已錨態免重查，只對殘留 [推論] 與
新前提跑升級——防雙稅）。→ 分流交棒：**多階段任務、需要 SDLC 級紀律**
...
很清楚** → 直接走 `to-prd`（mattpocock，無需訪談、直接把對話 synthesis 成 PRD）+ `implement`（消化
PRD/issue 直接執行，內建 tdd/code-review），不需六階段協議。交棒時對計劃形狀帶兩條
...
| 執行揭露計劃斷言不成立（斷言級證偽，非邊緣小偏離） | `loop-harness-standard/modules/execution-feedback.md`（N-diverse-variant → 判官逐斷言比對軌跡）：走該迴圈勾稽表確認是飄移還是誤讀，飄移 → plan-delta 人閘 |
...
| 斷言級勾稽（哪些計劃斷言被執行推翻） | `loop-harness-standard/modules/execution-feedback.md` 的 judge-verdict 勾稽表——比 quiz 更硬，人 review「HELD/REFUTED/UNOBSERVED」逐條 | 人理解半 |
| 對話式回報遺留問題 | `qa`（輕澄清 → 背景 Explore → 檔 domain-language issue） | 人理解半 |
| 產物本身的判準/驗證 tier（DR 報告 / Path-B 精煉 / COMPLETENESS 覆蓋矩陣） | **交棒 `judge-loop-chooser`**（**antigravity 本地 fork**：三態 grounding + 四層獨立性 tier）——按原則五 (1)：判官不得先消費 U0-U1 的 retrieval input 再判本產物，須 fresh 只對錨＋spec | 產物判別半 |
...
> **完成收斂（指針，2026-07-19）**：本 skill 主導的任務，「完成」判準＝oracle-aware 完成契約
> （trajectory-證 必要＋fold-in 條件性），非「路由走完＝完成」——與 `sdlc-plan-composer` 殊途同歸，
> 兩入口單一 SSOT → `loop-harness-standard` harness-spec §5.1（指針，不重抄）。
...
- **`sdlc-plan-composer`**：U1 出口只做「未知是否收斂完了」的判斷；計劃怎麼寫、S-1..S5 怎麼切是它的事，
  ...
  單階段仍走 `to-prd`+`implement`（本 skill 判斷交棒哪一個，不判斷計劃內容本身）。
- **`to-prd`+`implement`**：單階段/需求已清楚時的輕量出口，不需六階段協議，mattpocock 全局 skill 不依賴任何 plugin。

...
## 1. 為何多數內容不能逐字複製——skill-bettor 沒有集中式 Resolved ledger
...
skill-bettor 現有 root `AGENTS.md` 是 cross-host 工程 entry/種子，不是「Resolved」帳本，也**不該**把它
改造成同構的集中帳本——不是因為偷懶,是因為
...
|---|---|---|
| durable home = root `AGENTS.md`「Resolved」帳本(單一集中式,所有範疇共用一個檔案) | **拆成多路**:單一家族行為/eval 教訓 → `families/<family>/changelog/`;該家族子技能領域知識 → 子技能 `SKILL.md`+`references/`;跨家族 harness 工程教訓 → [`loop-harness-standard`](../../loop-harness-standard/SKILL.md) 自己的 Gotchas/modules;迴圈拓撲事實 → [`harness-wiki`](../../harness-wiki/SKILL.md) 組件卡;repo 級決策 → `ARCHITECTURE.md` §10/§11 | root `AGENTS.md` 雖存在，但不持有 Resolved ledger；更根本的是結構前提不同——skill-bettor 是多家族×多迴圈資產工場,教訓真實歸屬不是一維。 |
...
| 「動到大小迴圈八大基座」核對 N×M host×driver 全景＋設計規範差異表 | 核對 `loop-harness-standard` 的 paired B1-B8 manifest、`dual-loop-eight-base.md` 與防退化鐵律 | 不搬 antigravity 的平行 host config；skill-bettor 自己保留 macro carrier×small driver 觀測維度。fold 必同步機器 manifest、正負控制與 harness-wiki 全景，不能只改散文。 |
...
- `ARCHITECTURE.md` §10(行 152,「## 10. 遷移步驟」)與 §11(行 169,「## 11. 為何不」)——`grep`
  已驗證確切標題與行號,非憑印象引用。
- root `AGENTS.md` ——已驗證存在；它是 cross-host entry/工程原則種子，沒有集中式 `Resolved` ledger。
...
若哪天有人想把 skill-bettor 的多路 durable home 收斂回單一集中檔案(例如把 root `AGENTS.md` 改成 ledger),

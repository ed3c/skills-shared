...
## Not For
1. **一般本地 bug／測試失敗／stack trace** → `diagnose` / `superpowers:systematic-debugging`(重現→縮小→修；無 trace 接管、無畢業)。

...
1. **Clone 全歷史 + 定位置**：`bash kb-ingest/setup-repo.sh <repo_name> <clone_url>` 會 clone 到 `/Users/neon/antigravity/repo/<repo_name>/<repo_name>`（絕不 `--depth 1`；TARGET＝該 clone，basename＝slug）。**OUT＝`/Users/neon/antigravity/repo/<repo_name>/`**（durable，**絕不放 /tmp**；產物 `repo_wiki/`、`invariants/`、`_judge/` 都落此。`/repo/` 已 gitignore）。

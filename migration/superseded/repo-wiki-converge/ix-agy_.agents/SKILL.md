...
- **KB Ingest Python 環境依賴**：Ingest 腳本 `indexing.ingest_repodoc_cli` 依賴專屬的虛擬環境，必須指定使用 `/Users/neon/northstar/.venv/bin/python3`，且前置注入 `PYTHONPATH="/Users/neon/ix-agy"` 以便正確解析本地 Module。
- **Monorepo 子專案 Git 判定**：子專案目錄下通常無獨立 `.git` 資料夾，直接用 `[ -d "$TARGET/.git" ]` 判定會出錯，需改以 `{ [ -d "$TARGET/.git" ] || git -C "$TARGET" rev-parse --is-inside-work-tree; }` 相容 monorepo 版本控制。
- **大專案判官超時 (L1 Judge Timeout)**：大型專案或多頁 wiki (如 >15 頁) 於 Judge 審查時，`agy` 預設的 5 分鐘超時會導致指令無聲退出，必須在 `agy` 判官呼叫中顯式指定 `--print-timeout 30m` 以上。
- **0 頁面機械過關漏洞**：若作者因 API 或 sandbox mount 失敗產生了 0 個 markdown 頁面，機械驗證器 `verify-claims.sh` 會因無錯誤而回傳 `PASS`，但後續會因 gaps 檔案缺失陷入 Fatal 迴圈。因此在 `verify-claims` 前必須阻斷 `pages=0` 情況。

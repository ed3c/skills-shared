...
### 8.1 Ingest 直譯器與環境依賴
* **根因**：Ingest 腳本 `indexing/ingest_repodoc_cli.py` 內部會 import `chromadb` 等套件。若直接使用系統預設的 `python3`，會因為套件缺失導致 Ingest 崩潰。而 workspace 的依賴項被安裝在 `/Users/neon/northstar/.venv/bin/python3`。此外，Python 解析本地 `indexing` 模組時，必須依靠 `PYTHONPATH="/Users/neon/ix-agy"` 才能順利解析 import 路徑。
* **處置**：於執行 Ingest 時，必須將 Python 指向 `/Users/neon/northstar/.venv/bin/python3` 並前置 `PYTHONPATH="/Users/neon/ix-agy"`。

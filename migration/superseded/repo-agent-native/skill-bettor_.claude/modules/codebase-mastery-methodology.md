...
> 封頂)**一對一映**，是全平台無關的推理紀律。Step 3 依外部缺口類型路由本地 `external-verify`、
> `research` 或 `gemini-conversation-research`，但不得把 skill 存在誤寫成外部 engine 已本地化。配套提示詞 →
...
> 的本地 port；它提供 L1 openwiki 收斂入口，並可經本地 `kb-ingest`／RepoDoc lane 進 KG。
> 本層仍是 L2 source-grounded 不變量與 L3 隱含設計合約機制，兩者互補，不互相取代。
...
| Tier 1 | `grepai_search`／trace、Serena symbols/references(需 target canary，見 `../SKILL.md`) | 在已讀源碼上做導航；命中本身最高 B+ | 低；失效即退 Tier 0 |
| Tier 3 | `external-verify`／`research`／條件式 `gemini-conversation-research` | **只補具名外部缺口**(生態譜系、上游契約、跨 repo 對比) | 中至高；各自守來源與 external-engine receipt |
...
3. **Tier 0 起**：`FAMILY.yaml`(interface/metrics 性質)→`SKILL.md`(路由器承諾)→`shared/conventions.md`
   ...
   通過 target canary(見 `../SKILL.md` MCP 契約)。**禁跳層**到語義檢索之前先把骨架讀出來。
...
### Step 3 — 真外部缺口的 demand-pull 路由

Step 1+2 跑完後，剩下的⚠待確認項若是**外部**的(生態譜系、上游契約、跨 repo 對比)：
1. 先在「External Gaps」逐條記 `⚠ 需外部查證`，說明 source 為何不能回答。
2. post-cutoff／單一能力 claim 走 `external-verify`；一般背景 legwork 走 `research`。
3. 只有缺口需要保真 Gemini 對話、逐輪 Q&A 或多輪 DR gap-fill 時，才走本地
   `gemini-conversation-research`。若其 receipt 標 `external_engine_required`，就停在候選／外部執行需求，
   禁宣稱 browser／DR engine 已在 skill-bettor 本地跑通。
4. 任一路線的 findings 都要回 primary source 或 target source 複驗，才能寫回規格。
5. **守住紅線不變**：對可讀 target，若 Step 1+2 已把已知需求覆滿就**不該**為了「完整」去研究外部缺口
...
README-level 吸收的全部 answer-key 並超越它；**真外部缺口 demand-pull 路由**(`external-verify`／`research`／
條件式 `gemini-conversation-research`，守 external-engine receipt)，**evaluator-first 對 answer-key 計分並
回頭硬化提示詞**，**behavioral claim 完整 RIP 定案**(家族自己的 `evals/runner.py`/`selftest.sh` 當載具)。
...
它不執行 `kb-ingest/agy-pass.sh`，也不 ingest KG。它可餵 S0 SCOPE 與 answer-key candidate，不能直接
...
- 活基座：`grepai`／Serena(可選導航，需 target canary)、`external-verify`、`research`、條件式
  `gemini-conversation-research`、`families/pinescript-audit/evals/`(RIP 載具的本地
  worked instance)、`loop_wiki/engine.sh`(RIP 載具的另一本地 worked instance)。
- **邊界**：`gemini-conversation-research` 的外部 browser／DR engine 只有 receipt 證實後才算 runnable；
  sandcastle worked-example 仍是北極星專屬。

...
0. 落地路徑判斷（先做，決定三檔寫去哪）：
   ...
   - 否則（共享 harness 引擎、外部 repo）→ 寫到
     <out>/invariants/<slug>/specs/{同三檔}.md（out-scoped；durable <out> 與 <slug> 由呼叫者提供，
     例如外部 repo 可用 repo/<repo_name>/，本 repo 計畫可用 docs/plans/<date>-<topic>/）。
   不確定時向呼叫者確認，不擅自二選一。
...
3. Tier 0 起：FAMILY.yaml/SKILL.md（interface/路由承諾）→ shared/conventions.md（顯性契約）→ 子技能
   ...
   target canary——見 ../SKILL.md 的 MCP 契約；canary 不通時全程 ripgrep/Read，不因此卡住）。
4. 任何「無法附 file:line」的事實（外部生態 / 上游 wire protocol / 雲端 SDK 假定）一律標
   「⚠ 需人工二次確認」。這是誠實邊界，不是失敗。禁把 README / FAMILY.yaml / brief 行銷宣稱當已驗證事實。
5. 三檔各自帶可 ingest frontmatter：node_kind: RepoDoc、穩定 repo、title、commit、kind: specs、
   page_type: specs、covers。缺任一 load-bearing 欄位就 fail-loud，不得產不可入庫的半橋。
...
⚠ 需人工二次確認後，依缺口路由 external-verify / research / 條件式 gemini-conversation-research**。
後者若回 external_engine_required 就停在外部執行需求，禁宣稱已本地跑通；詳
codebase-mastery-methodology.md §3。
...
1. **鐵律 0 新增第 0 條路徑判斷**：antigravity 版寫死 `.knowledge_base/`(落在 target 自己的 repo 內)；
   ...
   out-scoped `specs/`)，理由見 `codebase-mastery-methodology.md` 的輸出位置設計註。
...
3. **範例從 ixsecurity/nats/gopush 換成 skill-bettor 本地可指的對象**(`families/pinescript-audit`／
   `loop_wiki/engine.sh`／`proposals/`／`agy` quota)；鐵律 6 補一句「真外部缺口標記即止」對應
   Step 3 的 demand-pull 外部缺口路由與 external-engine receipt。

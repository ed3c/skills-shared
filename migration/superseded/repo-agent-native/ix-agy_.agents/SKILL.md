...
## Gotchas
- **Ingestion Python Environment (PyEnv)**: Ingestion requires execution via `/Users/neon/northstar/.venv/bin/python3` and prepending `PYTHONPATH="/Users/neon/ix-agy"`. Default system python lacks the required dependency modules (like `chromadb`).
- **Monorepo Subproject Git Verification (Subproject Git)**: Subprojects of a parent monorepo lack local `.git/` directories. Verification scripts must use `{ [ -d "$TARGET/.git" ] || git -C "$TARGET" rev-parse --is-inside-work-tree; }` to check version-control scope.
- **Zero-Page Mechanical Pass Loophole (Zero-Page Check)**: Ensure `pages > 0` is verified before asserting mechanical PASS to prevent loops from silently spinning in a blank state.

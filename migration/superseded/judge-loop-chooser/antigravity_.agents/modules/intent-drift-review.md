...
> **封裝包 cross-plan scope 紀律（fold-in 2026-07-09）**：若原始意圖某錨由**姊妹計劃**的 artifact 服務（如某計劃唯讀消費 P2／kb-ingest），組裝封裝包時須在②/③附一句 **scope-boundary 註記**標明「錨 X 由 sibling-plan 服務、非本產物職責」——否則零存取 reviewer 會把「本產物未覆蓋 X」誤判為 **SI7 negative-space 假陽性**。實測：autoresearch A3 跨家族 Gemini 對 intent Round 2/3 錨（八大基座/sdlc 分階段）給 0.65 drift，第三路徑證實三分之二是 packet-scope 誤判（該三錨由 P2/計劃檔+執行服務、非 dossier 職責）。**assembler 加此註記＝把跨計劃邊界顯式化，reviewer 才不會抓假陽性。**

...
## 已查證真相表(2026-08-02 snapshot — MCP 2026-07-28 / Python SDK 2.0.0)

### #M1 `mcp==2.0.0` 是官方 Python SDK v2 stable release
- 判定:VERIFIED-primary
- 信心層:CONSENSUS(primary; GitHub release + PyPI verified metadata)
- 鐵錨 URL:`https://github.com/modelcontextprotocol/python-sdk/releases/tag/v2.0.0` + `https://pypi.org/project/mcp/2.0.0/`
- 關鍵事實:release page 標示 `v2.0.0 Latest`、stable v2、commit `6f69a37`、支援 MCP `2026-07-28` 與更早 revision;PyPI 頁顯示 `mcp 2.0.0`、`Requires: Python >=3.10`。
- 查證日期:2026-08-02
- 過期紀律:最新版本與 API 會變;引用前重跑 external-verify step 1-2。

### #M2 MCP `2026-07-28` 是 request-stateless，不是整個應用無 durable state
- 判定:VERIFIED-primary
- 信心層:CONSENSUS(primary)
- 鐵錨 URL:`https://modelcontextprotocol.io/specification/2026-07-28/basic#statelessness`
- 關鍵事實:server 不得從先前 request/connection 推導 context;每個 request 自帶 metadata。跨 request state 仍可存在，但必須由 client 在每個 request 顯式傳入 identifier。
- 走樣註記:「MCP stateless」不能推成「Registry、receipt、revocation、audit 皆不需持久化」。
- 查證日期:2026-08-02

### #M3 Python SDK v2.0.0 官方 CI pin conformance `0.2.0-alpha.10`
- 判定:VERIFIED-primary(config only)
- 信心層:CONSENSUS(primary config)
- 鐵錨 URL:`https://github.com/modelcontextprotocol/python-sdk/blob/v2.0.0/.github/workflows/conformance.yml`
- 關鍵事實:`CONFORMANCE_PKG` pin `@modelcontextprotocol/conformance@0.2.0-alpha.10`，並有 client/server `--spec-version 2026-07-28` legs。
- 限制:這只證明官方 release tag 的 CI 設定，**不等於本 repo 已通過 conformance**。後續本地實跑結果見 M5；client checks 含 skip，嚴格 repo gate 仍未通過。
- 查證日期:2026-08-02

### #M4 本機精確 SDK 已通過 in-process modern path，但不是 wire conformance
- 判定:VERIFIED-local-partial
- 信心層:DETERMINISTIC(local package metadata + executable test)
- 物理鐵錨:`mcp==2.0.0` METADATA SHA-256 `761c76bdad3a786ecf0b7c7d9c48b0fe7bd0e3ce085ab388a622bde4dbee81f9`; `@modelcontextprotocol/conformance==0.2.0-alpha.10` package SHA-256 `29ef755c66311589bf731763045790aba83adaa462334363c5edad194aa4420b`。
- 已證:官方 `Client(mode="auto")` 對 repo adapter 完成 `server/discover`、`tools/list`、`tools/call(resolve_skill)`，且 canonical decision bytes 與 JSONL oracle 相同。
- 後續:真實 Streamable HTTP 與 official client/server 已在外部執行層實跑，結果見 M5；multi-worker restart 與 strict zero-skip client admission 仍未證。
- 查證日期:2026-08-02

### #M5 官方 conformance 對 repo candidate endpoint 已實跑，但 strict production admission 未過
- 判定:VERIFIED-local-partial
- 信心層:DETERMINISTIC(精確安裝的官方 conformance CLI + 實際 repo candidate HTTP endpoint)
- 物理鐵錨:`@modelcontextprotocol/conformance==0.2.0-alpha.10`;server checks SHA-256 `2e7bd6a0b7398d1750b6b3e247b98222f8777fbf4133946d94dc54e3e65e4c30`;client checks SHA-256 `e2f06918ae5c17a3a78827d626fea1e32e44fa919799db147a2e3fc1e18b615a`。
- 已證:官方 `server-stateless` scenario 對 `http://127.0.0.1:8000/mcp` 為 `30 SUCCESS / 30`、exit 0；官方 `request-metadata` client CLI 為 `Passed: 5/5`、`OVERALL: PASSED`、exit 0。
- 限制/反證:client `checks.json` 同時包含 `3 SKIPPED`(roots、sampling、elicitation capability declaration)。repo `_verify_conformance_checks` 只接受 `SUCCESS/INFO`，因此 strict `client-and-server-passed` **未成立**，`production_adapter_enabled` 必須維持 false。candidate 使用 production-identical transport policy，但附加 test-only diagnostics；不能把它當 production admission receipt。
- 未證:zero-skip client result、TCP multi-worker kill/restart/replay、完整外部簽章 admission chain。
- 查證日期:2026-08-02
